...
const ReportsView = () => {
  ...
  const [activeTimeFilter, setActiveTimeFilter] = useState(
    todayTimeFilter as Date[] | null[]
  );
  /* TIME FILTER VALUE BEFORE APPLYING CUSTOM DATE RANGE */
  const [previousTimeFilter, setPreviousTimeFilter] = useState([null, null] as
    | Date[]
    | null[]);
  /* CUSTOM TIME RANGE FILTER VALUE */
  const [customTimeFilter, setCustomTimeFilter] = useState([null, null] as
    | Date[]
    | null[]);
  ...
  function applyNewCustomTF() {
    if (previousTimeFilter[0] === null && previousTimeFilter[1] === null)
      setPreviousTimeFilter(activeTimeFilter);
