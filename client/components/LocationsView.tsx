...
export default function LocationsView() {
  ...
  async function refreshLocations() {
    await fetchLocations().then((data) =>
      setLocations(data as unknown as number[][])
    );
  }
    ...
    <div className={styles.viewContainer}>
      ...
      <section className={styles.cardSection}>
        {locations.length > 0 && locations[0] !== undefined
          ? locations.map((item) => {
              return (
                <LocationCard key={randomAlphaNumeric()} location={item} />
              );
            })
          : undefined}
      </section>
  ...
  return (
    <>
      <FloatButton
        ...
        icon={<PlusOutlined />}
        tooltip={<div>Add a new location</div>}
        className={styles.addBtn}
      />
      {newLocationModal ? (
        <Modal
          open={newLocationModal}
          onCancel={handleLocationModal}
          footer={[
            <Button key={randomAlphaNumeric()} onClick={handleLocationModal}>
              Done
            </Button>,
          ]}
        >
